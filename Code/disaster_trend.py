# -*- coding: utf-8 -*-
"""Disaster_trend.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1f5siPHBqCouCSB7K54BfbB8aTJldu6hL
"""
...
# Function to forecast future values for a disaster type (without comparing with actual values)
def forecast_future(disaster_data, disaster_type, target_column, steps=10):
    # Prepare the time-series data (e.g., severity, casualties, etc.)
    ts_data = disaster_data[target_column]

    # Split into train set (here, using all data for training)
    train = ts_data

    # ARIMA Model
    model = ARIMA(train, order=(5, 1, 0))  # You can tune the order as needed
    model_fit = model.fit()

    # Make future predictions (without actual comparison)
    future_predictions = model_fit.forecast(steps=steps)

    # Generate future dates for the predictions
    last_date = ts_data.index[-1]
    future_dates = pd.date_range(last_date, periods=steps+1, freq='H')[1:]
    ...
    # Export the future predictions to CSV
    output_filename = f'{disaster_type}_{target_column}_future_predictions.csv'
    future_df.to_csv(output_filename, index=False)
    print(f"Future predictions saved to {output_filename}")

    # Plot the predictions
    plt.figure(figsize=(10, 6))
    plt.plot(ts_data.index, ts_data.values, label='Historical Data')
    plt.plot(future_df['Date'], future_df['Predicted'], label='Future Predictions', color='red')
    plt.title(f'ARIMA Future Prediction for {disaster_type} - {target_column}')
    plt.xlabel('Date')
    plt.ylabel(target_column)
    plt.legend()
    plt.show()

# Example usage: for each disaster type, forecast future values for severity, casualties, economic loss, and duration
for disaster_type, subset in disaster_data.items():
    for target_column in ['severity', 'casualties', 'economic_loss_million_usd', 'duration_hours']:
        forecast_future(subset, disaster_type, target_column, steps=10)  # Predict the next 10 steps (hours)
